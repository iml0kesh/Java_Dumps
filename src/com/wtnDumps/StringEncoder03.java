...
public class StringEncoder03 {
    public static void main(String[] args) {
        ...
        for(int i=0; i<s.length; i++){
            if(s[i].length()%3==0){
                int n = s[i].length()/3;
                s1f += s[i].substring(0,n);
                s1m += s[i].substring(n,s[i].length()-n);
                s1l += s[i].substring(s[i].length()-n);
            }
            if(s[i].length()%3==1){
                int n = s[i].length()/3;
                s2f += s[i].substring(0,n);
                s2m += s[i].substring(n,s[i].length()-n);
